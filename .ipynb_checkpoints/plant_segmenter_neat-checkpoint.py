...
def _convex_hull_fill(mask_bool: np.ndarray):
    ...
    hull_mask = np.zeros_like(mask_bool, dtype=np.uint8)
...
def predict_extend_mask(mask_bool: np.ndarray, method="auto", strength=1.0, forbid_mask: np.ndarray | None = None):
    """
    method: 'auto' | 'rosette' | 'blade'
    strength: 0.5..1.5 (scales extension for 'blade'; small dilation for 'rosette')
    forbid_mask: if provided, predicted result is clipped to avoid other masks.
    """
    ys, xs = np.nonzero(mask_bool)
    if xs.size < 3:
        return mask_bool
    ...
    if method == "auto":
        xspan = xs.max() - xs.min() + 1
        yspan = ys.max() - ys.min() + 1
        elong = max(xspan, yspan) / max(1.0, min(xspan, yspan))
        method = "blade" if elong >= 1.6 else "rosette"

    if method == "rosette":
        out = _convex_hull_fill(mask_bool)
        # small grow if user bumps strength
        if strength > 1.01:
            k = int(round(2 * (strength - 1.0)))
            if k > 0:
                se = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2*k+1, 2*k+1))
                out = cv2.morphologyEx(out.astype(np.uint8), cv2.MORPH_DILATE, se).astype(bool)
    else:
        out = _tapered_extension(mask_bool, k_extend=0.6 * float(strength))

    if forbid_mask is not None:
        out = np.logical_and(out, ~forbid_mask)
    return out
...
class LeafSegmenterGUI:
    ...
    def make_masks_frame(self, root):
        ...
        ttk.Button(bar, text="Extend/Predict", command=self.on_predict_extend).pack(side="left", padx=(6, 0))

        ttk.Label(bar, text="Mode:").pack(side="left", padx=(8,2))
        self.extend_mode = tk.StringVar(value="auto")
        ttk.Combobox(bar, width=9, state="readonly",
                    textvariable=self.extend_mode,
                    values=("auto", "rosette", "blade")).pack(side="left") #can add a strength knob later, we can add a tiny Spinbox; default 1.0 is fine for now
    ...
    def _rebuild_mask_list(self):
        ...
        for i, m in enumerate(self.sr.masks):
            tag = " [PRED]" if m.get("meta", {}).get("predicted") else ""
    ...
    def on_predict_extend(self):
        ...
        if len(sel) != 1:
            ...
            return
        idx = sel[0]
        base = self.sr.masks[idx]
        base_bool = base["segmentation"].astype(bool)

        # avoid invading other segments
        forbid = np.zeros_like(base_bool, dtype=bool)
        for j, m in enumerate(self.sr.masks):
            if j != idx:
                forbid |= m["segmentation"].astype(bool)

        mode = (self.extend_mode.get() or "auto").lower()
        pred_bool = predict_extend_mask(base_bool, method=mode, strength=1.0, forbid_mask=forbid)

        # extended region only
        extended_only = np.logical_and(pred_bool, ~base_bool)

        # compute bbox
        ys, xs = np.nonzero(pred_bool)
        if xs.size:
            x1, x2 = int(xs.min()), int(xs.max())
            y1, y2 = int(ys.min()), int(ys.max())
            bbox = [x1, y1, x2 - x1 + 1, y2 - y1 + 1]
        ...
        new_mask = {
            "segmentation": pred_bool.astype(np.uint8),
            "bbox": bbox,
            "area": float(pred_bool.sum()),
            "meta": {
                "predicted": True,
                "pred_mode": mode,
                # store the added part so we can draw outline and keep color stats clean
                "extended_bool": extended_only.astype(np.uint8)
            }
        }
        self.sr.masks.append(new_mask)
        self._rebuild_mask_list()
        self.lb.selection_clear(0, tk.END)
        self.lb.selection_set(tk.END)  # focus the new one
        self.on_select_mask()          # refresh preview
    ...
    def on_select_mask(self, event=None):
        ...
        if ext_full is not None:
            ext_full = (ext_full.astype(np.uint8) > 0)
            x, y, w, h = map(int, m["bbox"])
            x2, y2 = x + w, y + h
            ext_crop = ext_full[y:y2, x:x2]
            base_crop = np.logical_and(msk, ~ext_crop)

            # contours
            comp_bgr = cv2.cvtColor(comp, cv2.COLOR_RGB2BGR)
            for arr, col, thick in (
                (base_crop, (0, 220, 255), 2),   # yellow = original visible part
                (ext_crop,  (255, 0, 255), 2),   # magenta = added/extended part
            ):
                cnts, _ = cv2.findContours(arr.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                cv2.drawContours(comp_bgr, cnts, -1, col, thick)
    ...
    def _measure_one_mask(self, idx: int, flags: dict):
        """
        Geometry (area/length/width) -> computed on the FULL current mask (incl. predicted extension).
        Color/HSV -> computed ONLY on the visible part (excludes 'extended_bool' if present).
        """
        rgb, mask, bbox = self._mask_and_rgb_for_idx(idx)  # mask is the current (maybe-extended) crop
        res = {}

        # ---- choose masks for geometry vs color ----
        geom_mask = mask.astype(bool)  # full (extended) for area/length/width
        color_mask = geom_mask.copy()  # start same, then remove hallucinated pixels if we have them

        # If this mask was extended, meta["extended_bool"] holds the *full-image* boolean of the added part.
        mrec = self.sr.masks[idx]
        meta = mrec.get("meta", {})
        ext_full = meta.get("extended_bool", None)
        if ext_full is not None:
            x, y, w, h = map(int, mrec["bbox"])
            x2, y2 = x + w, y + h
            # crop the extended-only map to this mask's bbox so it aligns with 'mask'
            ext_crop = (ext_full.astype(np.uint8) > 0)[y:y2, x:x2]
            if ext_crop.shape == geom_mask.shape:
                color_mask = np.logical_and(geom_mask, ~ext_crop)

        # guard: if everything was hallucinated (rare), fall back so stats don’t blow up
        if color_mask.sum() == 0:
        ...
        if flags.get("length") or flags.get("width"):
            ...
            if flags.get("width"):
                res["width_row_max_px"] = round(paper["width_px"], 2)
