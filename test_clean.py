#!/usr/bin/env python3
"""
Clean simple test - NO bounding boxes, just leaf outlines.
- Solid green = visible leaf boundary
- Dotted magenta = predicted complete boundary
"""
import sys
from typing import Tuple
import numpy as np
import cv2
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))
from leaf_completion_v2 import UNetCompletion, letterbox, unletterbox, auto_generate_mask
import torch


def predict_with_padding(model, rgb, visible_mask, size, device, pad_percent=0.5):
    """Predict with padding to allow extension."""
    h, w = rgb.shape[:2]
    pad_h = int(h * pad_percent)
    pad_w = int(w * pad_percent)

    # Padded image (white background)
    new_h = h + 2 * pad_h
    new_w = w + 2 * pad_w
    padded_rgb = np.full((new_h, new_w, 3), 255, dtype=np.uint8)
    padded_rgb[pad_h:pad_h+h, pad_w:pad_w+w] = rgb

    # Padded visible mask
    padded_vis = np.zeros((new_h, new_w), dtype=np.uint8)
    padded_vis[pad_h:pad_h+h, pad_w:pad_w+w] = visible_mask

    # Letterbox and predict
    rgb_sq, info = letterbox(padded_rgb, size, pad_value=255)
    vis_sq, _ = letterbox(padded_vis, size, pad_value=0)

    model.eval()
    with torch.no_grad():
        rgb_t = torch.from_numpy(rgb_sq.astype(np.float32) / 255.0).permute(2, 0, 1)
        vis_t = torch.from_numpy(vis_sq[None, ...].astype(np.float32) / 255.0)
        x = torch.cat([rgb_t, vis_t], dim=0).unsqueeze(0).to(device)
        logits = model(x)
        probs = torch.sigmoid(logits)[0, 0].cpu().numpy()

    pred_sq = (probs >= 0.5).astype(np.uint8) * 255
    pred_padded = unletterbox(pred_sq, info)

    return pred_padded, padded_rgb, padded_vis, (pad_h, pad_w, h, w)


def draw_dotted_line(img, pt1, pt2, color, thickness=2, gap=8):
    """Draw a dotted line between two points."""
    dist = np.sqrt((pt2[0] - pt1[0])**2 + (pt2[1] - pt1[1])**2)
    if dist < 1:
        return
    n_segments = int(dist / gap)
    if n_segments < 1:
        n_segments = 1

    for i in range(0, n_segments, 2):
        t1 = i / n_segments
        t2 = min((i + 1) / n_segments, 1.0)
        x1 = int(pt1[0] + t1 * (pt2[0] - pt1[0]))
        y1 = int(pt1[1] + t1 * (pt2[1] - pt1[1]))
        x2 = int(pt1[0] + t2 * (pt2[0] - pt1[0]))
        y2 = int(pt1[1] + t2 * (pt2[1] - pt1[1]))
        cv2.line(img, (x1, y1), (x2, y2), color, thickness)


def draw_dotted_contour(img, contour, color, thickness=2, gap=8):
    """Draw a dotted contour."""
    pts = contour.reshape(-1, 2)
    for i in range(len(pts)):
        pt1 = tuple(pts[i])
        pt2 = tuple(pts[(i + 1) % len(pts)])
        draw_dotted_line(img, pt1, pt2, color, thickness, gap)


def main():
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", required=True)
    parser.add_argument("--images", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--device", default="mps")
    args = parser.parse_args()

    print(f"Loading model: {args.model}")
    ckpt = torch.load(args.model, map_location="cpu", weights_only=False)
    size = ckpt.get("size", 256)

    model = UNetCompletion(in_ch=4, base_ch=32)
    model.load_state_dict(ckpt["state_dict"])

    device = args.device
    if device == "mps" and not torch.backends.mps.is_available():
        device = "cpu"
    model.to(device)

    images_dir = Path(args.images)
    exts = {".png", ".jpg", ".jpeg"}
    image_paths = [p for p in images_dir.iterdir() if p.suffix.lower() in exts]

    out_dir = Path(args.output)
    out_dir.mkdir(parents=True, exist_ok=True)

    all_results = []

    for img_path in image_paths:
        print(f"Processing: {img_path.name}")

        rgb = cv2.imread(str(img_path), cv2.IMREAD_COLOR)
        rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)

        visible_mask = auto_generate_mask(rgb)

        # Predict with padding
        pred_full, padded_rgb, padded_vis, pad_info = predict_with_padding(
            model, rgb, visible_mask, size, device, pad_percent=0.5
        )

        # Create clean visualization - just the leaf on white with outlines
        canvas = padded_rgb.copy()

        # Get contours
        vis_contours, _ = cv2.findContours(padded_vis, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        pred_contours, _ = cv2.findContours(pred_full, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # Draw predicted outline as DOTTED MAGENTA
        for cnt in pred_contours:
            if cv2.contourArea(cnt) > 100:  # Skip tiny noise
                draw_dotted_contour(canvas, cnt, (255, 0, 255), thickness=3, gap=6)

        # Draw visible outline as SOLID GREEN
        for cnt in vis_contours:
            if cv2.contourArea(cnt) > 100:
                cv2.drawContours(canvas, [cnt], -1, (0, 180, 0), 2)

        all_results.append(canvas)

        # Save individual
        out_path = out_dir / f"{img_path.stem}_completed.png"
        cv2.imwrite(str(out_path), cv2.cvtColor(canvas, cv2.COLOR_RGB2BGR))

    # Make grid
    if all_results:
        # Find max dimensions
        max_h = max(r.shape[0] for r in all_results)
        max_w = max(r.shape[1] for r in all_results)

        # Pad all to same size
        padded = []
        for r in all_results:
            p = np.full((max_h, max_w, 3), 255, dtype=np.uint8)
            y_off = (max_h - r.shape[0]) // 2
            x_off = (max_w - r.shape[1]) // 2
            p[y_off:y_off+r.shape[0], x_off:x_off+r.shape[1]] = r
            padded.append(p)

        # 2 columns layout
        n = len(padded)
        cols = 2
        rows = (n + cols - 1) // cols

        grid_rows = []
        for row in range(rows):
            row_imgs = []
            for col in range(cols):
                idx = row * cols + col
                if idx < n:
                    row_imgs.append(padded[idx])
                else:
                    row_imgs.append(np.full((max_h, max_w, 3), 255, dtype=np.uint8))
            grid_rows.append(np.concatenate(row_imgs, axis=1))

        grid = np.concatenate(grid_rows, axis=0)

        grid_path = out_dir / "results_grid.png"
        cv2.imwrite(str(grid_path), cv2.cvtColor(grid, cv2.COLOR_RGB2BGR))
        print(f"\nSaved: {grid_path}")

    print("Done!")
    print(f"Results in: {out_dir}")


if __name__ == "__main__":
    main()
