...
class LeafSegmenterGUI:
    def __init__(self, root):
        ...
        # UI vars for the Leaf Completion tab (occlusion augmentation + completion model)
        self.occ_data_var = tk.StringVar(value="")
        self.occ_masks_var = tk.StringVar(value="")
        ...
        self.occ_edge_bias_var = tk.DoubleVar(value=0.0)
        self.occ_edge_band_var = tk.IntVar(value=8)
        self.occ_edge_only_var = tk.BooleanVar(value=False)

        # Completion model (no SAM)
        self.comp_out_var = tk.StringVar(value=str(Path.home() / "leaf_completion.pth"))
        self.comp_steps_var = tk.IntVar(value=5000)
        self.comp_lr_var = tk.DoubleVar(value=1e-4)
        self.comp_size_var = tk.IntVar(value=512)
        self.comp_batch_var = tk.IntVar(value=2)
        self.comp_device_var = tk.StringVar(value="cpu")
        self.comp_arch_var = tk.StringVar(value="unet_resnet18")
        self.comp_pretrained_var = tk.BooleanVar(value=True)
        self.comp_edge_w_var = tk.DoubleVar(value=0.2)
        self.comp_resume_var = tk.BooleanVar(value=False)
        self.comp_val_images_var = tk.StringVar(value="")
        self.comp_val_masks_var = tk.StringVar(value="")
        self.comp_preview_every_var = tk.IntVar(value=200)
        self.comp_preview_n_var = tk.IntVar(value=3)
        self.comp_preview_live_var = tk.BooleanVar(value=True)
    ...
    def make_masks_frame(self, paned_parent):
        ...
        btn_extend = ttk.Button(bar, text="Complete", width=8, command=self.on_complete_selected_mask, style='Icon.TButton')
        btn_extend.pack(side="left", padx=(4, 0))
        ToolTip(btn_extend, "Complete selected mask (uses completion model)")
    ...
    def make_training_frame(self, parent):
        """Create training panel with tabs: Fine-tune, Leaf Completion, and Target Segment."""
        ...
        # ════════════════════════════════════════════════════════════════════
        tab1 = self._make_scrollable_tab(notebook, "  Fine-tune  ")
        ...
        # ════════════════════════════════════════════════════════════════════
        # TAB 2: Leaf Completion (Occlusion + Completion Model)
        # ════════════════════════════════════════════════════════════════════
        tab2 = self._make_scrollable_tab(notebook, "  Leaf Completion  ")

        # Description
        desc = ttk.Label(tab2, text="Train a completion model to predict full leaf masks from partially occluded masks.\n"
                                    "Uses synthetic occlusions during training (leaf-over-leaf, edge-biased).",
        ...
        # --- Masks folder (optional)
        r += 1
        ttk.Label(tab2, text="Leaf masks folder:").grid(row=r, column=0, sticky="e", padx=(0,4))
        ttk.Entry(tab2, textvariable=self.occ_masks_var, width=50).grid(row=r, column=1, sticky="ew", padx=4)
        ttk.Button(tab2, text="Choose...", command=lambda: self._browse_folder_into(self.occ_masks_var)).grid(row=r, column=2)
        ttk.Button(tab2, text="Check pairs", command=self._check_occ_pairs).grid(row=r, column=3, padx=(6,0))

        # --- Optional validation data for live preview
        r += 1
        ttk.Label(tab2, text="Test images folder (optional):").grid(row=r, column=0, sticky="e", padx=(0,4))
        ttk.Entry(tab2, textvariable=self.comp_val_images_var, width=50).grid(row=r, column=1, sticky="ew", padx=4)
        ttk.Button(tab2, text="Choose...", command=lambda: self._browse_folder_into(self.comp_val_images_var)).grid(row=r, column=2)

        r += 1
        ttk.Label(tab2, text="Test masks folder (optional):").grid(row=r, column=0, sticky="e", padx=(0,4))
        ttk.Entry(tab2, textvariable=self.comp_val_masks_var, width=50).grid(row=r, column=1, sticky="ew", padx=4)
        ttk.Button(tab2, text="Choose...", command=lambda: self._browse_folder_into(self.comp_val_masks_var)).grid(row=r, column=2)

        # --- Occlusion augmentation parameters
        r += 1
        params_frame = ttk.LabelFrame(tab2, text=" Occlusion Augmentation ", padding=8)
        params_frame.grid(row=r, column=0, columnspan=3, sticky="ew", pady=(8,4))

        ttk.Label(params_frame, text="Min occlusion %:").grid(row=0, column=0, sticky="e", padx=(0,4))
        ttk.Scale(params_frame, from_=0.05, to=0.50, variable=self.occ_min_var, orient="horizontal", length=120).grid(row=0, column=1, sticky="w")
        self.occ_min_lbl = ttk.Label(params_frame, text="15%", width=5)
        self.occ_min_lbl.grid(row=0, column=2, sticky="w")

        ttk.Label(params_frame, text="Max occlusion %:").grid(row=1, column=0, sticky="e", padx=(0,4))
        ttk.Scale(params_frame, from_=0.20, to=0.80, variable=self.occ_max_var, orient="horizontal", length=120).grid(row=1, column=1, sticky="w")
        self.occ_max_lbl = ttk.Label(params_frame, text="50%", width=5)
        self.occ_max_lbl.grid(row=1, column=2, sticky="w")

        ttk.Label(params_frame, text="Occluder count min:").grid(row=2, column=0, sticky="e", padx=(0,4))
        ttk.Spinbox(params_frame, from_=0, to=5, textvariable=self.occ_count_min_var, width=5).grid(row=2, column=1, sticky="w")

        ttk.Label(params_frame, text="Occluder count max:").grid(row=3, column=0, sticky="e", padx=(0,4))
        ttk.Spinbox(params_frame, from_=1, to=10, textvariable=self.occ_count_max_var, width=5).grid(row=3, column=1, sticky="w")

        ttk.Label(params_frame, text="Edge bias:").grid(row=4, column=0, sticky="e", padx=(0,4))
        ttk.Scale(params_frame, from_=0.0, to=1.0, variable=self.occ_edge_bias_var, orient="horizontal", length=120).grid(row=4, column=1, sticky="w")
        self.occ_edge_bias_lbl = ttk.Label(params_frame, text="0%", width=5)
        self.occ_edge_bias_lbl.grid(row=4, column=2, sticky="w")

        ttk.Label(params_frame, text="Edge band (px):").grid(row=5, column=0, sticky="e", padx=(0,4))
        ttk.Spinbox(params_frame, from_=2, to=64, increment=2, textvariable=self.occ_edge_band_var, width=5).grid(row=5, column=1, sticky="w")
        ttk.Checkbutton(params_frame, text="Edge-only occlusion", variable=self.occ_edge_only_var)\
        ...
        def update_occ_labels(*_):
            ...
            self.occ_edge_bias_lbl.configure(text=f"{int(self.occ_edge_bias_var.get()*100)}%")
        self.occ_min_var.trace_add("write", update_occ_labels)
        self.occ_max_var.trace_add("write", update_occ_labels)
        self.occ_edge_bias_var.trace_add("write", update_occ_labels)

        # --- Completion model (no SAM)
        r += 1
        comp = ttk.LabelFrame(tab2, text=" Completion Model (no SAM) ", padding=8)
        comp.grid(row=r, column=0, columnspan=3, sticky="ew", pady=(8,4))

        ttk.Label(comp, text="Save to (.pth):").grid(row=0, column=0, sticky="e", padx=(0,4))
        ttk.Entry(comp, textvariable=self.comp_out_var, width=50).grid(row=0, column=1, sticky="ew", padx=4)
        ttk.Button(comp, text="...", command=lambda: self._browse_save_into(self.comp_out_var, default_ext=".pth")).grid(row=0, column=2)

        ttk.Label(comp, text="Steps:").grid(row=1, column=0, sticky="e", padx=(0,4))
        ttk.Spinbox(comp, from_=1000, to=200000, increment=1000, textvariable=self.comp_steps_var, width=8).grid(row=1, column=1, sticky="w")
        ttk.Label(comp, text="LR:").grid(row=1, column=2, sticky="e", padx=(8,4))
        ttk.Entry(comp, textvariable=self.comp_lr_var, width=10).grid(row=1, column=3, sticky="w")

        ttk.Label(comp, text="Image size:").grid(row=2, column=0, sticky="e", padx=(0,4))
        ttk.Spinbox(comp, from_=256, to=1024, increment=64, textvariable=self.comp_size_var, width=8).grid(row=2, column=1, sticky="w")
        ttk.Label(comp, text="Batch:").grid(row=2, column=2, sticky="e", padx=(8,4))
        ttk.Spinbox(comp, from_=1, to=16, increment=1, textvariable=self.comp_batch_var, width=8).grid(row=2, column=3, sticky="w")

        ttk.Label(comp, text="Device:").grid(row=3, column=0, sticky="e", padx=(0,4))
        ttk.Combobox(comp, textvariable=self.comp_device_var, values=["cuda", "mps", "cpu"], width=8).grid(row=3, column=1, sticky="w")
        ttk.Label(comp, text="Arch:").grid(row=3, column=2, sticky="e", padx=(8,4))
        ttk.Combobox(
            comp,
            textvariable=self.comp_arch_var,
            values=[
                "unet_resnet18",
                "unet_resnet34",
                "unet_resnet50",
        ...
        ttk.Checkbutton(comp, text="Pretrained encoder", variable=self.comp_pretrained_var).grid(row=3, column=4, sticky="w", padx=(6,0))

        ttk.Label(comp, text="Edge loss weight:").grid(row=4, column=0, sticky="e", padx=(0,4))
        ttk.Entry(comp, textvariable=self.comp_edge_w_var, width=8).grid(row=4, column=1, sticky="w")
        ttk.Checkbutton(comp, text="Resume", variable=self.comp_resume_var).grid(row=4, column=2, sticky="w", padx=(8,0))

        ttk.Label(comp, text="Preview every (steps):").grid(row=5, column=0, sticky="e", padx=(0,4))
        ttk.Spinbox(comp, from_=0, to=100000, increment=50, textvariable=self.comp_preview_every_var, width=8)\
            .grid(row=5, column=1, sticky="w")
        ttk.Label(comp, text="Preview count:").grid(row=5, column=2, sticky="e", padx=(8,4))
        ttk.Spinbox(comp, from_=1, to=16, increment=1, textvariable=self.comp_preview_n_var, width=6)\
            .grid(row=5, column=3, sticky="w")
        ttk.Checkbutton(comp, text="Live preview in main panel", variable=self.comp_preview_live_var)\
            .grid(row=5, column=4, sticky="w", padx=(8,0))

        comp_bar = ttk.Frame(comp)
        comp_bar.grid(row=6, column=0, columnspan=5, sticky="w", pady=(6,0))
        ttk.Button(comp_bar, text="Train Completion Model", command=self._launch_completion_training).pack(side="left")
        ttk.Button(comp_bar, text="Load Completion Model", command=self._load_completion_model).pack(side="left", padx=(8,0))
        ttk.Button(comp_bar, text="Stop Training", command=self._stop_training).pack(side="left", padx=(8,0))
        ...
        # ════════════════════════════════════════════════════════════════════
        tab3 = self._make_scrollable_tab(notebook, "  Target Segment  ")
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Leaf Completion (Occlusion Augmentation + Completion Model) Methods
    # ════════════════════════════════════════════════════════════════════════
    ...
    def _candidate_stems_for_image(self, stem: str):
        stems = {stem}
        if "_crop_" in stem:
    ...
    def _build_mask_index(self, masks_dir: Path):
        exts = (".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp")
        index = {}
    ...
    def _check_occ_pairs(self):
        images_dir = self.occ_data_var.get().strip()
        masks_dir = self.occ_masks_var.get().strip()
        if not images_dir or not os.path.isdir(images_dir):
            messagebox.showwarning("Pairs", "Pick a valid leaf images folder first.")
            return
        if not masks_dir or not os.path.isdir(masks_dir):
            messagebox.showwarning("Pairs", "Pick a valid leaf masks folder first.")
            return

        exts = (".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp")
        img_paths = [p for p in Path(images_dir).rglob("*") if p.is_file() and p.suffix.lower() in exts]
        mask_index = self._build_mask_index(Path(masks_dir))

        missing = []
        for p in img_paths:
            stems = self._candidate_stems_for_image(p.stem)
            if not any(s in mask_index for s in stems):
                missing.append(p.name)

        if not missing:
            messagebox.showinfo("Pairs", f"All good! {len(img_paths)} images have matching masks.")
            return

        preview = "\n".join(missing[:20])
        msg = f"Missing masks for {len(missing)} images.\n\nFirst 20:\n{preview}"
        messagebox.showwarning("Pairs", msg)
        try:
            self._append_train_log("Missing masks:")
            for name in missing:
                self._append_train_log(f"  {name}")
        except Exception:
            pass

    def _launch_completion_training(self):
        """Launch leaf_completion_trainer.py (no SAM) for shape completion."""
        images_dir = self.occ_data_var.get().strip()
        masks_dir = self.occ_masks_var.get().strip()
        if not images_dir or not os.path.isdir(images_dir):
            messagebox.showwarning("Completion", "Pick a valid folder with leaf images.")
            return
        if not masks_dir or not os.path.isdir(masks_dir):
            messagebox.showwarning("Completion", "Pick a valid folder with leaf masks.")
            return

        out_path = self.comp_out_var.get().strip() or str(Path.home() / "leaf_completion.pth")
        steps = int(self.comp_steps_var.get())
        lr = float(self.comp_lr_var.get())
        img_size = int(self.comp_size_var.get())
        batch = int(self.comp_batch_var.get())
        device = self.comp_device_var.get().strip() or "cpu"
        arch = (self.comp_arch_var.get().strip() or "unet_resnet18")
        edge_w = float(self.comp_edge_w_var.get())
        pretrained = bool(self.comp_pretrained_var.get())
        resume = bool(self.comp_resume_var.get()) and bool(out_path) and os.path.exists(out_path)
        ...
        edge_bias = float(self.occ_edge_bias_var.get())
        edge_band = int(self.occ_edge_band_var.get())
        edge_only = bool(self.occ_edge_only_var.get())

        val_images = self.comp_val_images_var.get().strip()
        val_masks = self.comp_val_masks_var.get().strip()
        preview_every = int(self.comp_preview_every_var.get())
        preview_n = int(self.comp_preview_n_var.get())
        preview_out = Path(tempfile.gettempdir()) / "leaf_completion_previews"
        use_preview = preview_every > 0 and val_images and val_masks and os.path.isdir(val_images) and os.path.isdir(val_masks)

        script_path = Path(__file__).parent / "leaf_completion_trainer.py"
        if not script_path.exists():
            messagebox.showerror("Completion", f"Cannot find leaf_completion_trainer.py at:\n{script_path}")
            return

        py = shlex.quote(sys.executable)
        cmd = (
            f"{py} {shlex.quote(str(script_path))} "
            f"--images {shlex.quote(images_dir)} "
            ...
            f"--edge-bias {edge_bias:.2f} "
            f"--edge-band {edge_band} "
            f"{'--edge-only ' if edge_only else ''}"
            f"{'--pretrained ' if pretrained else ''}"
            f"{f'--resume {shlex.quote(out_path)} ' if resume else ''}"
        )

        if use_preview:
            cmd += (
                f"--val-images {shlex.quote(val_images)} "
                f"--val-masks {shlex.quote(val_masks)} "
                f"--preview-every {preview_every} "
                f"--preview-count {preview_n} "
                f"--preview-out {shlex.quote(str(preview_out))} "
            )
        elif preview_every > 0 and (val_images or val_masks):
            self._append_train_log("NOTE: Preview disabled (test images/masks folders not both set).")

        self._append_train_log("")
        self._append_train_log("=" * 60)
        self._append_train_log("Launching Completion Model Training (no SAM):")
        self._append_train_log(f"  Images: {images_dir}")
        self._append_train_log(f"  Masks: {masks_dir}")
        self._append_train_log(f"  Output: {out_path}")
        self._append_train_log(f"  Steps: {steps}, LR: {lr}, Size: {img_size}, Batch: {batch}, Device: {device}")
        self._append_train_log(f"  Arch: {arch}, Pretrained: {pretrained}, Edge loss: {edge_w}")
        self._append_train_log(f"  Occlusion: {occ_min*100:.0f}%-{occ_max*100:.0f}%, {occ_count_min}-{occ_count_max} occluders")
        self._append_train_log(f"  Edge bias: {edge_bias:.0%}, Edge band: {edge_band}px")
        if edge_only:
            self._append_train_log("  Edge-only occlusion: True")
        if resume:
            self._append_train_log("  Resume: True")
        if use_preview:
            self._append_train_log(f"  Preview: every {preview_every} steps, {preview_n} sample(s)")
        self._append_train_log("=" * 60)
        self._append_train_log(f"Command: {cmd}")
        self._append_train_log("")

        try:
            self._train_job_name = "Completion"
            ...
            self.set_status("Completion model training started...", "info")
        except Exception as e:
            messagebox.showerror("Completion", f"Failed to start training:\n{e}")

    def _load_completion_model(self):
        out_path = self.comp_out_var.get().strip()
        if not out_path or not os.path.exists(out_path):
            messagebox.showwarning("Completion", "Pick a valid completion model (.pth) file.")
            return
        try:
            from leaf_completion_model import load_completion_checkpoint
            map_dev = self.comp_device_var.get().strip() or "cpu"
            model, meta = load_completion_checkpoint(out_path, device=map_dev)
            self.comp_model = model
            self.comp_meta = meta or {}
            self._append_train_log(f"Loaded completion model from: {out_path}")
            messagebox.showinfo("Completion Model", f"Completion model loaded.\n\n{out_path}")
            self.set_status("Completion model loaded", "success")
        except Exception as e:
            messagebox.showerror("Completion", f"Failed to load completion model:\n{e}")
    ...
    def _show_preview_from_path(self, path: str):
        try:
    ...
    def _occ_train_reader_thread(self):
        """Read output from the active training subprocess."""
        p = getattr(self, "_occ_train_proc", None)
        if not p:
            return
        for raw in iter(p.stdout.readline, b""):
            ...
            if line.startswith("PREVIEW:"):
                ...
                if bool(self.comp_preview_live_var.get()):
                    self.root.after(0, lambda p=preview_path: self._show_preview_from_path(p))
            self.root.after(0, lambda s=line: self._append_train_log(s))
        p.wait()
        code = p.returncode
        job = getattr(self, "_train_job_name", "Training")
        self.root.after(0, lambda: self._append_train_log(f"\n{job} training finished with code {code}"))
        if code == 0:
            self.root.after(0, lambda: self.set_status(f"{job} training completed!", "success"))
        else:
    ...
    def on_complete_selected_mask(self):
        """Complete selected mask using completion model (no SAM)."""
        sel = list(self.lb.curselection())
        if len(sel) != 1:
            messagebox.showwarning("Pick one", "Select exactly one mask to complete.")
            return
        if self.comp_model is None:
            messagebox.showwarning("Completion model", "Load the completion model first.")
            return
        ...
        self.sr.masks.append({
            ...
            "bbox": bbox,
            "area": float(pred.sum()),
            "meta": {"predicted": True, "pred_mode": "completion", "extended_bool": added.astype(np.uint8)}
        })
        self._rebuild_mask_list()
        self.lb.selection_clear(0, tk.END)
        self.lb.selection_set(tk.END)
    ...
    def _predict_extend_ml(self, base_mask_bool: np.ndarray, forbid_mask: np.ndarray | None):
        # Prefer completion model if loaded
        if getattr(self, "comp_model", None) is not None:
    ...
    def _predict_extend_completion(self, base_mask_bool: np.ndarray, forbid_mask: np.ndarray | None):
        try:
            from leaf_completion_model import predict_completion_mask
        except Exception as e:
            messagebox.showwarning("Completion", f"Completion model not available: {e}")
            return None

        if self.comp_model is None:
            messagebox.showwarning("Completion", "Load the completion model first.")
            return None
        base_img = self._base_image()
        if base_img is None:
            return None

        meta = self.comp_meta or {}
        input_size = int(meta.get("input_size", int(self.comp_size_var.get() or 512)))
        thr = float(meta.get("threshold", 0.5))
        device = next(self.comp_model.parameters()).device

        pred = predict_completion_mask(
            self.comp_model,
            base_img,
            base_mask_bool.astype(np.uint8),
            input_size=input_size,
            device=str(device),
            threshold=thr,
        ).astype(bool)

        # Postprocess: keep the base exactly, but clean up the "added" pixels.
        # This prevents the common "speckle" effect where the model predicts tiny boundary dots.
        pred = np.logical_or(pred, base_mask_bool)
        if forbid_mask is not None:
            pred = np.logical_and(pred, ~forbid_mask)

        added = np.logical_and(pred, ~base_mask_bool)
        if added.any():
            # Remove pepper noise and thin spurs; do not erode the base.
            k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
            added_u8 = added.astype(np.uint8) * 255
            added_u8 = cv2.morphologyEx(added_u8, cv2.MORPH_OPEN, k, iterations=1)

            # Drop very small blobs (relative to base size) that survive opening.
            num, labels = cv2.connectedComponents((added_u8 > 0).astype(np.uint8), connectivity=4)
            if num > 1:
                base_area = int(base_mask_bool.sum())
                min_blob = max(30, int(0.003 * base_area))  # ~0.3% of base, at least 30 px
                keep = np.zeros_like(base_mask_bool, dtype=bool)
                for lbl in range(1, num):
                    comp = labels == lbl
            ...
            pred = np.logical_or(base_mask_bool, added)

        # Ensure we never return disconnected junk.
        pred = self._keep_components_touching_base(pred, base_mask_bool)
        return pred
