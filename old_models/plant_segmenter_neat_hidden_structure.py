...
class LeafSegmenterGUI:
    def __init__(self, root):
        ...
        self.occ_resume_var = tk.BooleanVar(value=False)
    ...
    def make_training_frame(self, parent):
        ...
        # --- Resume
        r += 1
        ttk.Checkbutton(tab2, text="Resume from existing hidden model", variable=self.occ_resume_var)\
        ...
        ttk.Label(params_frame, text="Edge bias:").grid(row=4, column=2, sticky="e", padx=(20,4))
        ttk.Scale(params_frame, from_=0.0, to=1.0, variable=self.occ_edge_bias_var, orient="horizontal", length=100).grid(row=4, column=3, sticky="w")
        self.occ_edge_bias_lbl = ttk.Label(params_frame, text="0%", width=5)
        self.occ_edge_bias_lbl.grid(row=4, column=4, sticky="w")

        ttk.Label(params_frame, text="Edge band (px):").grid(row=5, column=2, sticky="e", padx=(20,4))
        ttk.Spinbox(params_frame, from_=2, to=64, increment=2, textvariable=self.occ_edge_band_var, width=5).grid(row=5, column=3, sticky="w")
        ...
        def update_occ_labels(*_):
            ...
            self.occ_edge_bias_lbl.configure(text=f\"{int(self.occ_edge_bias_var.get()*100)}%\" if hasattr(self, \"occ_edge_bias_lbl\") else \"\")
    ...
    def _launch_occlusion_training(self):
        ...
        resume = bool(self.occ_resume_var.get()) and bool(out_path) and os.path.exists(out_path)
        ...
        if resume:
            cmd += f" --resume {shlex.quote(out_path)}"
